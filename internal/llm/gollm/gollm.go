...
// "mistral", "ollama"). Returns a typed error when required config is missing
...
func NewRunner(cfg model.LLMConfig, opts ...Option) (*Runner, error) {
	...
	// Base-URL override for the OpenAI-compatible provider. Providers with
	// their own URL ignore the setting.
	if cfg.Endpoint != "" {
